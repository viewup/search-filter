...
if ( ! class_exists( 'SearchAndFilter' ) ) {
	class SearchAndFilter {
		...
		public function __construct() {
			...
			$this->frmqreserved = array(
				SF_FPRE . "category_name",
				SF_FPRE . "s",
				SF_FPRE . "tag",
				SF_FPRE . "submitted",
				SF_FPRE . "post_date",
		...
		public function shortcode( $atts, $content = null ) {
			...
			$defaultConfigs = array(
				'permalink'          => true,
				// if set, Removes the permalink behavior for taxonomies and make redirect to the page set. if 0, the root page
				'fields'             => null,
				'taxonomies'         => null,
				//will be deprecated - use `fields` instead
				'submit_label'       => null,
				'submitlabel'        => null,
				//will be deprecated - use `submit_label` instead
				'search_placeholder' => esc_attr_x( 'Search &hellip;', 'placeholder' ),
				'types'              => "",
				'type'               => "",
				//will be deprecated - use `types` instead
				'headings'           => "",
				'all_items_labels'   => "",
				'class'              => "",
				'post_types'         => "",
			...
			// get the shortcode configs
			$configs = shortcode_atts( $defaultConfigs, $atts );
			...
			$permalink = $permalink === true || $permalink === '' ? true : $permalink;
		...
		function filter_query_post_meta( $query ) {
			...
			foreach ( $_GET as $field => $value ) {
				...
				if ( ! $value || strpos( $field, SF_METAPRE ) !== 0 ) {
					continue;
				}
		...
		function fix_blank_search( $query ) {//needs to be re-implemented
		...
		public function set_defaults() {
			...
			foreach ( $wp_query->meta_query->queries as $key => $query ) {
				...
				if ( ! in_array( SF_METAPRE . $query['key'], $this->taxonomylist ) ) {
					continue;
				}
		...
		public function check_posts() {
			...
			$permalinkOption = ! $allowPermalink ? $_POST[ SF_FPRE . 'permalink' ] : false;
			...
			if ( $this->has_form_posted ) {//if the search has been posted, redirect to the newly formed url with all the right params
				...
				if ( $permalinkOption !== false ) {
					if ( is_numeric( $permalinkOption ) ) {
						$permalinkOption = (int) $permalinkOption;
						if ( $permalinkOption ) {
							$base_redirect = get_permalink( $permalinkOption );
						}
					} else {
						$base_redirect = $permalinkOption;
					}
				}
				wp_redirect( ( $base_redirect . $this->urlparams ) );
				exit;
			}
		}
		...
		public function get_search_filter_form( $submitlabel, $search_placeholder, $fields, $types, $labels, $hierarchical, $hide_empty, $show_count, $post_types, $order_by, $order_dir, $operators, $all_items_labels, $empty_search_url, $add_search_param, $class, $permalink ) {
			...
			if ( ! in_array( "post_types", $fields ) ) {//then the user has not added it to the fields list so the user does not want a post types drop down... so add (if any) the post types to a hidden attribute
			...
			if ( $permalink !== true ) {
				$returnvar .= "<input type=\"hidden\" name=\"" . SF_FPRE . "permalink\" value=\"{$permalink}\" />";
			}
